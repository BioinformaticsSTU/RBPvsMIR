...
commands.getoutput("rm "+input_TS_utr);
############
...
file_TS="result/"+title+"_"+num+"/TS_output.txt";
...
file_result="result/"+title+"_"+num+"/PRresult_TS.txt";
...
for i in range(0,num_R):
    for j in range(0,num_TS):
        if( ( int(arrR[i][1])<=int(arrTS[j][1]) and int(arrTS[j][1])<=int(arrR[i][2]) ) or ( int(arrR[i][1])<=int(arrTS[j][2]) and int(arrTS[j][2])<=int(arrR[i][2]) ) ):
            fp_w.write(arrR[i][0]+"\t"+str(arrR[i][1])+"-"+str(arrR[i][2])+"\t"+str(arrR[i][3])+"\t"+str(arrR[i][4])+"\t"+arrTS[j][0]+"\t"+str(arrTS[j][1])+"-"+str(arrTS[j][2])+"\n");
...
#######
file_result="result/"+title+"_"+num+"/PRresult_pita.txt";
fp_w=open(file_result, "w");
for i in range(0,num_R):
