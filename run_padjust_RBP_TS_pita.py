...
file_pita="result/"+num+"/test_pita_results.tab";
...
while(line_pita!=""):
    ...
    ddG=words_pita[12].strip();
    arrP.append([]);
    arrP[i]=[utr, mirna, start, end, ddG];
    line_pita=fp_pita.readline();
    i=i+1;
...
file_result="result/"+num+"/PRresult_TS.txt";
fp_w_TS=open(file_result, "w");
for i in range(0,num_R):
    for j in range(0,num_TS):
        if( ( int(arrR[i][1])<=int(arrTS[j][1]) and int(arrTS[j][1])<=int(arrR[i][2]) ) or ( int(arrR[i][1])<=int(arrTS[j][2]) and int(arrTS[j][2])<=int(arrR[i][2]) ) ):
            fp_w_TS.write(arrR[i][0]+"\t"+arrTS[j][0]+"\t"+str(arrR[i][1])+"-"+str(arrR[i][2])+"\t"+ str(arrTS[j][1])+"-"+str(arrTS[j][2])+"\t"+arrR[i][3]+"\n" );


fp_w_TS.close();

#########
file_result="result/"+num+"/PRresult_pita.txt";
fp_w_pita=open(file_result, "w");
for i in range(0,num_R):
    for j in range(0,num_P):
        if( ( int(arrR[i][1])<=int(arrP[j][2]) and int(arrP[j][2])<=int(arrR[i][2]) ) or ( int(arrR[i][1])<=int(arrP[j][3]) and int(arrP[j][3])<=int(arrR[i][2]) ) ):
            fp_w_pita.write(arrR[i][0]+"\t"+arrP[j][1]+"\t"+str(arrR[i][1])+"-"+str(arrR[i][2])+"\t"+ str(arrP[j][2])+"-"+str(arrP[j][3])+"\t"+arrR[i][3]+"\t"+arrP[j][4]+"\n" );
